...
const DarkTheme = createContext(true);
...
function App() {
  ...
  return (
    <DarkTheme.Provider value={darkTheme}>
      <div className={`${darkTheme ? 'dark' : ''} overflow-hidden text-xl dark:bg-gradient-to-br dark:from-white dark:to-lightGray h-screen w-full grid grid-cols-4 grid-rows-6 justify-items-center items-start`}>
              ...
              {books.length === 0 ? (
                <>
                  ...
                  <button 
                    onClick={addNewBook} >
                    <svg 
                      className={`h-6 relative top-px dark:stroke-lightGray dark:hover:stroke-white stroke-darkGray hover:stroke-black  transition-all duration-300 hover:rotate-90 overflow-hidden ${book.title === '' ? 'w-0' : 'w-6'}`}
                ...
                <>
                  <h3 className='font-bold bg-transparent text-right'>
                    {currentBook}
                  </h3>
                  <svg 
                    onMouseEnter={handleDropdownHover}
                    className='w-8 relative top-px dark:fill-white fill-darkGray transition-all duration-300 hover:fill-black hover:rotate-180 hover:-translate-y-1' 
